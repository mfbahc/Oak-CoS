...
This matrix keeps public claims honest. A capability is marked as implemented, scaffolded, optional connector-backed, or future/private extension.
...
| Daily brief | Scaffolded | `core/skills/daily-brief/SKILL.md`, `core/templates/daily-brief.md`, `core/routines/daily-brief.md` | demo output, template checks, first local brief check |
...
| Gmail inbox work | Optional connector-backed | `core/connectors/gmail.md`, inbox triage skills and templates | connector docs and no-send defaults |
| Google Calendar agenda work | Optional connector-backed | `core/connectors/google-calendar.md`, meeting prep skill | connector docs and no-write defaults |
| Google Drive document work | Optional connector-backed | `core/connectors/google-drive.md` | connector docs and no-share defaults |
| Granola transcript import | Optional connector-backed | `core/connectors/granola.md`, transcript ingest template | connector docs and no-share defaults |
| Slack summaries and drafts | Optional connector-backed | `core/connectors/slack.md` | connector docs and no-send defaults |
| Browser automation | Optional connector-backed | `core/connectors/browser.md` | connector docs and disabled-by-default checks |
| WhatsApp, Telegram, Discord bridges | Optional connector-backed | `core/connectors/whatsapp.md`, `core/connectors/telegram.md`, `core/connectors/discord.md` | connector docs and disabled/read-draft defaults |
...
Public Oak should not imply that optional connector-backed capabilities are active just because they are documented or selected in config. They become active only after the user connects the provider in the runtime and grants access.
